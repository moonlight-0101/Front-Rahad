...
const ClockBox = ({ mainTime, setTime }) => {
    ...
    const handelTimeAction = ({ event, mode }) => {
        const mainEventTime = event.target.value; // Minute
        if (
            (mode === "min" && /^[0-5]?[0-9]$/.test(mainEventTime)) ||
            mainEventTime === ""
        ) {
            setTime((prev) => ({ ...prev, min: mainEventTime })); // Update the state only if the input is valid or empty
        } // Hour
        if (
            (mode === "hour" && /^(0?\d|1\d|2[0-3])$/.test(mainEventTime)) ||
            mainEventTime === ""
        ) {
    ...
    return (
        <div className="w-[320px] sm:w-[420px] rounded-[9px] px-12 m-auto flex flex-row-reverse items-center gap-5 justify-center  border border-[rgb(162,175,184)]">
            <div>
                
                <input
                    onChange={(e) =>
                        handelTimeAction({ event: e, mode: "hour" })
                    }
                    value={mainTime?.hour}
                    placeholder="12"
                    className="w-[92px] rounded-sm h-[57px] border border-black/10 mb-1 mt-4 
                ...
                <div className=" text-left ml-2 text-[#49454F] text-[12px]">
                    ساعت
                </div>
            ...
            <div>
                ...
                <input
                    type="text"
                    onChange={(e) =>
                        handelTimeAction({ event: e, mode: "min" })
                    }
                    value={mainTime?.min}
                    placeholder="00"
                    className="w-[92px] rounded-sm h-[57px] border border-black/10 mb-1 mt-4 
